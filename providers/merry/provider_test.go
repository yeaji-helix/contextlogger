...
func TestMerryTraceback(t *testing.T) {
	...
	Expect(output.String()).To(MatchRegexp(`time=sometime level=error msg="it broke" how=badly ~stackTrace=".*yeaji-helix/contextlogger/providers/merry.*"`))
}
...
func TestErrorTraceback(t *testing.T) {
	...
	testProvider.Error(context.Background(), false, err)
	Expect(output.String()).To(MatchRegexp(`time=sometime level=error msg="it broke" ~stackTrace=".*yeaji-helix/contextlogger/providers/merry.*"`))
}
